"""
CrimeLens AI — Crime Signature Engine v2 Demo

Generates fully formed, highly nested CrimeSignatureV2 JSON payloads from
mock IngestedCase records to demonstrate the new canonical data representation.
"""
...
def main():
    print("CrimeLens AI — Initializing Signature Engine v2 Demo...\n")
    ...
    case_2 = IngestedCase(
        ...
        latitude=13.0000,
        longitude=77.6000,
        brief_facts="Accused attacked the victim with an iron rod near the highway.",
    ...
    for case in cases:
        print(f"Building Signature v2 for Crime No: {case.crime_no} ({case.crime_major_head})")
        
        # Build Signature
        sig_v2 = builder.build_from_case(case)
        
        # Serialize
        json_data = SignatureSerializer.to_json(sig_v2)
        
        # Write to file
        output_file = os.path.join(output_dir, f"signature_v2_{case.case_master_id}.json")
        with open(output_file, "w") as f:
            f.write(json_data)
            
        print(f"  -> Generated Semantic Text: '{sig_v2.embedding_document.semantic_text}'")
        print(f"  -> Saved to {output_file}\n")

    print(f"Successfully generated 3 CrimeSignatureV2 JSON payloads in '{output_dir}'.")
