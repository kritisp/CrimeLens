"""
CrimeLens AI — PostgreSQL / Supabase Repository

Concrete implementation of the FIRDataSource repository contract using
SQLAlchemy AsyncSession to perform optimized queries against PostgreSQL (Supabase).
"""
...
class PostgresFIRRepository(FIRDataSource):
    """
    SQLAlchemy-backed async repository targeting PostgreSQL (Supabase).
    Handles CRUD operations for raw FIRs and their nested relationships.
    """
    ...
    async def fetch_raw_fir(self, case_master_id: int) -> Dict[str, Any]:
        """Fetches raw FIR by ID with eager-loaded relationships."""
        stmt = (
            select(DbCaseMaster)
    ...
    async def list_raw_firs(self) -> List[Dict[str, Any]]:
        """Retrieves all stored case records with eager-loaded relationships."""
        stmt = (
            select(DbCaseMaster)
            .options(
                selectinload(DbCaseMaster.complainant),
    ...
    async def store_raw_fir(self, case_data: Dict[str, Any]) -> int:
        """Saves a new case record mapping nested structures to database tables."""
        ...
        def _naive_dt(dt_val: Any) -> Any:
            if hasattr(dt_val, "tzinfo") and dt_val is not None:
        ...
        db_case = DbCaseMaster(
            ...
            incident_date_from=_naive_dt(case_data["incident_date_from"]),
            incident_date_to=_naive_dt(case_data.get("incident_date_to")),
            info_received_ps_date=_naive_dt(case_data["info_received_ps_date"]),
